...
public class GooseAnimation extends JFrame {
	...
	public void setScene(SimpleUniverse su) {
		ObjectFile f = new ObjectFile(ObjectFile.RESIZE);
		Scene gooseScene = null;
		try {
			gooseScene = f.load("source_folder/Goose.obj");
		} catch (Exception e) {
		...
		setAppearenceToDefault(bodyApp, new Color3f(0.6f, 0.6f, 0.6f));
		Shape3D body = (Shape3D) gooseNamedObjects.get("body");
		body.setAppearance(bodyApp);

		Appearance orangeApp = new Appearance();
		setAppearenceToDefault(orangeApp, new Color3f(250 / 255f, 160 / 255f, 9 / 255f));
	...
	private static void setAppearenceToDefault(Appearance app, Color3f col) {
		app.setMaterial(new Material(col, col, col, col, 150.0f));
	}

	private void setLight(SimpleUniverse su) {
